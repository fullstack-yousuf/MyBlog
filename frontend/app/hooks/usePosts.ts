...
export const useUpdatePost = () => {
  ...
  return useMutation({
    mutationFn: async (data: { id: string; title?: string; content?: string }) => {
      return api.put(`/api/posts/${data.id}`, {
        title: data.title,
...
export const useDeletePost = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async (id: string) => api.delete(`/api/posts/${id}`),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["posts"] });
      qc.invalidateQueries({ queryKey: ["myPosts"] });
    },
...
export const useLikePost = () => {
    const qc = useQueryClient();
    return useMutation({
      mutationFn: async (id: string) => api.post(`/api/posts/${id}/like`),
      onSuccess: (_, id) => {
qc.invalidateQueries({ queryKey: ["posts"] });
...
export const useComment = () => {
  ...
  return useMutation({
    mutationFn: async (payload: { id: string; text: string }) =>
      api.post(`/api/posts/${payload.id}/comment`, { text: payload.text }),
    onSuccess: (_, payload) => {
